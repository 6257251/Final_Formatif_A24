    ...
    private async Task EvaluateChoices()
    {
        using (IServiceScope scope = _serviceScopeFactory.CreateScope())
        {
            ...
            foreach (var userId in _data.Keys)
            {
                ...
                {
                    Player? player = await dbContext.Player.SingleOrDefaultAsync(x => x.UserId == userId);
                    if (player != null)
                    {
                ...
                {
                   await _mathQuestionHub.Clients.User(userId).SendAsync("BadAnswer", _currentQuestion.Answers[_currentQuestion.RightAnswerIndex]);
                }
